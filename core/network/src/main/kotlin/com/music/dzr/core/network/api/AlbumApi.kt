...
internal interface AlbumApi {
     ...
     */
    @GET("album/{id}")
    suspend fun getAlbum(@Path("id") albumId: Long): NetworkResponse<Album>

    /**
     * Retrieves all tracks from a specific album.
     */
    @GET("album/{id}/tracks")
    suspend fun getAlbumTracks(@Path("id") albumId: Long): NetworkResponse<PaginatedList<AlbumTrack>>
}